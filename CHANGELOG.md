...
### Fixed
- `serverInfo.version` reported by the MCP server was lagging behind `package.json` (showed `1.2.0` while npm published `1.2.1`). Now correctly reports `1.2.2`.

### Added (repo infra, not shipped in the npm tarball)
- **Dockerfile** + `.dockerignore` for [Glama](https://glama.ai) MCP server indexing (Glama uses it to render the server card).
...
### Added
- **MCP Registry manifest** ([`server.json`](server.json)) describing the package, stdio transport, and environment variables (`MS_CLIENT_ID`, `MS_TENANT`). Enables submission to the official [Model Context Protocol Registry](https://registry.modelcontextprotocol.io/) at `io.github.MAG-Cie/mcp-microsoft-todo`.
